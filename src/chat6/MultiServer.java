...
public class MultiServer {
		...
		try {
			...
			*/
			while (true) {
				...
				/*클라이언트 1명당 하나의 쓰레드가 생성되어 메시지 전송 및
				 수신을 담당한다.*/
	...
	public void sendAllMsg(String name, String msg) {
		...
		while (it.hasNext()) {
			try {
				...
				/*
				클라이언트에게 메세지를 전달할때 매개변수로 name이 있는 경우와
				없는 경우를 구분해서 전달한다.
				*/
				if (name.equals("")) {
					/*입장 혹은 퇴장에서 사용되는 부분*/
	...
	class MultiServerT extends Thread {
		...
		public void run() {
			...
			try {
				/* 클라이언트가 보내는 최초메세지는 대화명이므로 접속에 대한 부분을
				 출력하고 Echo 한다. */
				//클라이언트 이름을 읽어온다.
				name = in.readLine();
				...
				/*두번째 입력부터는 메세지이므로 모든 클라이언트에게 Echo 한다.*/
				while (in != null) {
					s = in.readLine();
					if (s == null)
						break;
				...
				finally절로 진입하게 된다. 이때 "대화명"을 통해 Map에서
				클라이언트의 정보를 제거한다.
				*/
