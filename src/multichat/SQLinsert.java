	...
	public void dbExecute() {
		try {
			...
			query = "INSERT INTO chat_talking values " + 
					" ('chat_serial_num.nextval', '1111','테스터1',sysdate)";
			//쿼리문 실행
			...
			query = "INSERT INTO chat_talking VALUES " + " (?,?,?,sysdate)";
			//동적쿼리 실행을 위한 preparedStatement 인스턴스 생성
			psmt = con.prepareStatement(query);
			/*
			동적쿼리문의 ?부분(인파라미터)을 하나씩 사용자의 입력값으로 채워준다.
			DB에서는 인덱스가 1부터 시작이므로 ?의 갯수만틈 순서대로 값을
			설정하면 된다. 
			*/
			...
			//insert 쿼리문이므로 성공시 1, 실패시 0을 반환된다.
